package fr.aliptic.formation.java2020.miniprojet.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "miniprojet-ear.ear");

        // Current directory is the root of the "miniprojet-web" project. Go up one level
        // and enter the "miniprojet-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../miniprojet-ejb/target/miniprojet-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "miniprojet-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/miniprojet-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "miniprojet-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("fr.aliptic.formation.java2020.miniprojet.test");
